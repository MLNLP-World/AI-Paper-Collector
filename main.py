...
def main():
    ...
    while 1:
        ...
        if special_input(query) or query == "":
            continue

        print("\n[+] Select search mode:\n\t[1] Exact\n\t[2] Fuzzy")
        select_mode = input("[+] Enter a number between 1 to 2: ")
        if special_input(select_mode, mode="select_mode"):
            continue
        if select_mode == "":
            select_mode = "1"
        ...
        if mode == "fuzzy":
            ...
            if special_input(threshold, mode="threshold"):
                continue
            if threshold == "":
                threshold = None
            else:
            ...
            if special_input(limit, mode="limit"):
                continue
            if limit == "":
                limit = None
            else:
