...
def set_args():
    ...
    parser.add_argument(
        "--mode", "-m", type=str, default="exact", choices=["fuzzy", "exact"], help="search mode"
    )
    ...
    parser.add_argument(
        "--force", "-f", action="store_true", help="force to update the cache file incrementally"
    )
...
def main():
    ...
    results = exec_search(
        indexes, candidates, args.query, args.mode, args.threshold, args.conf, args.limit
    )
